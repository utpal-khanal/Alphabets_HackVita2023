...
{
	[SerializeField] Convert convert;
	...
	public void SetMyImage(Image t)
	{
		tempImage = t;
	}
	...
	public void ProcessAndSaveAll()
	{
		...
		FileHelper.SaveBytesToFile(FileHelper.MakePath("Assets/Processed Bytes"), saveFileName.ToString(), allBytes.ToArray(), true);
            FileHelper.SaveBytesToFile(FileHelper.MakePath("Assets/Processed Bytes"), saveFileName.ToString() + "_labels", allLabelBytes.ToArray(), true);

		Debug.Log("completion successful");
	...
	public TransformationSettings CreateRandomSettings(System.Random rng, Image image)
	{
		...
		//without random settings

		settings.angle = 0;
		settings.scale = 1;

		settings.noiseSeed = 0;
